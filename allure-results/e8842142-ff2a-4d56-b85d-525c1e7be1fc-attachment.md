...
- Name: ✅ Users Module Tests >> ❌ Invalid Name Test #3: Selam Endet6neh
- Location: C:\Users\derej\Desktop\Connect\tests\bankPortal\Maker\users.spec.js:56:5
...
Call log:
  - waiting for getByRole('button', { name: 'Create User' })

    at UsersPage.openCreateUserModal (C:\Users\derej\Desktop\Connect\pages\bankPortal\maker\users.page.js:40:33)
    at UsersPage.createUserWithInvalidName (C:\Users\derej\Desktop\Connect\pages\bankPortal\maker\users.page.js:137:16)
    at C:\Users\derej\Desktop\Connect\tests\bankPortal\Maker\users.spec.js:61:23
```
...
>  40 |     await this.createUserButton.click();
  ...
  128 |     await this.realmOptionMaker.click();
  129 |   }
  130 |
  131 |   async submitUserForm() {
  132 |     await this.saveButton.waitFor({ state: "visible", timeout: 5000 });
  133 |     await this.saveButton.click();
  134 |   }
  135 |
  136 |   async createUserWithInvalidName(user) {
  137 |     await this.openCreateUserModal();
  138 |     await this.fillUserFields(user);
  139 |     await this.submitUserForm();
  140 |   }
```